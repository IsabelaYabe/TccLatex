...
A implementação do \textit{Src2MoF} apoia-se em três pilares tecnológicos: 
(i) \textbf{Eclipse} (IDE e ecossistema MDE) para implementar o gerador de modelos e as transformações \emph{Text-to-Model} \emph{(A Model Driven Reverse Engineering Framework for Generating High Level UML Models From Java Source Code: ``In Src2MoF, Eclipse tool is used, which facilitates the Intermediate Model Discoverer (IMD) implementation and UML2 modeling. Our model generator is using Text-to-model transformations which is also implemented in Eclipse.''} :contentReference[oaicite:6]{index=6}); 
(ii) o \textbf{parser de código-fonte Java} (\JavaParser) integrado ao \emph{Intermediate Model Discoverer} (IMD) para produzir uma AST e um \emph{Intermediate Model} textual em XML \emph{(A Model Driven Reverse Engineering Framework for Generating High Level UML Models From Java Source Code: ``Source code parser uses an open source tool `Java Parser' … it parses the source code and passes an abstract syntax tree AST to model discovery phase … Implemented IMD is generic and language independent … This intermediate representation is an XML like depiction of source code.''} :contentReference[oaicite:7]{index=7} :contentReference[oaicite:8]{index=8}); 
e (iii) a plataforma \textbf{\UMLtwo} sobre \textbf{\EMF} para materializar os modelos UML de alto nível (diagramas de classes e de atividades) e para a qual os dois \emph{templates} (\emph{Class Diagram Creator} e \emph{Activity Diagram Creator}) foram escritos \emph{(A Model Driven Reverse Engineering Framework for Generating High Level UML Models From Java Source Code: ``UML2 is an Eclipse Modeling Framework (EMF) based implementation of UML 2.x … Transformation templates i.e. class diagram creator and activity diagram creator are written for UML2.''} :contentReference[oaicite:9]{index=9}).

Especificamente, o \emph{model generator} “\emph{calls two templates i.e. class diagram creator and activity diagram creator}” e seu “\emph{Implementation … is carried out using Eclipse tool}” \emph{(A Model Driven Reverse Engineering Framework for Generating High Level UML Models From Java Source Code)}. :contentReference[oaicite:10]{index=10}

Na avaliação, os autores também mencionam ferramentas de modelagem usadas para a comparação manual (Papyrus, StarUML, Rational Rose, editor UML do Eclipse), ainda que não façam parte do \emph{pipeline} automático do \textit{Src2MoF} \emph{(A Model Driven Reverse Engineering Framework for Generating High Level UML Models From Java Source Code: ``experts have manually performed modeling … by means of different software design tools i.e. Eclipse, papyrus, starUML, rationalrose''}. :contentReference[oaicite:11]{index=11}
...
Na validação, os autores comparam a saída do \textit{Src2MoF} com modelos elaborados manualmente por especialistas em ferramentas como Eclipse, Papyrus, StarUML e Rational Rose. Trata-se de uma evidência \emph{empírica e qualitativa} baseada em julgamento de especialistas, sem métricas quantitativas de acurácia (p.~ex., precisão/recall) ou tempo de execução.

A avaliação ocorre em duas etapas: (1) modelagem manual (diagramas de classes e de atividades) por engenheiros de software; (2) execução do \textit{Src2MoF} nos mesmos casos; (3) comparação entre as saídas. São cinco estudos de caso de referência; dois são detalhados no artigo: \textit{Automated Teller Machine (ATM)} e \textit{Amadeus Hospitality}.

Os diagramas de classes gerados refletem atributos, operações e relacionamentos observados no código.

Para cada método, é criada uma \texttt{Activity}; o corpo da operação é traduzido para \texttt{OpaqueAction}, com uso de \texttt{CallOperationAction}, \texttt{CreateObjectAction}, nós inicial/final, nós condicionais e fluxos de controle/objeto. 

\paragraph{Síntese (RQ6).}
\emph{Método:} comparação especialista (modelos manuais vs.\ gerados). 
\emph{Escopo:} 5 estudos de caso (ATM e Amadeus descritos).
\emph{Qualidade prática:} conformidade percebida de estrutura e comportamento; \emph{sem} métricas quantitativas. 
\emph{Ameaças:} linguagem (Java), suporte parcial em \textit{Activity}, generalização limitada.

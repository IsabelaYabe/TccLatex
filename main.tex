...
\begin{itemize}
  ...
  \item Condensing Class Diagrams With Minimal Manual Labeling Cost (2016).
...
\subsection{Síntese comparativa dos estudos}
...
Java (OSS) &
UML Classe condensado (classes “importantes”) &
\Static; \Comp\ (redução cognitiva) &
Métricas (size, coupling, network) $\rightarrow$ \textit{k}-means (amostragem) $\rightarrow$ ensemble (Random Forest) $\Rightarrow$ seleção de classes &
MCCondenser; SDMetrics; MagicDraw &
OSS (9 projetos; 2640 classes); AUC=0.73; +10–20\% vs. baselines; treino ≈1s; predição <0,1s; rótulos 10\% \\
% foi
...
Java; OO geral &
UML Sequência &
Estático; Compreensão/Redocumentação (comportamento) &
Código $\rightarrow$ AST (JavaParser); DFS pós-ordem; regras para \emph{calls}, \emph{object creation}, \emph{return}, \emph{alt}/\emph{loop}; geração com PlantUML &
REVUML; JavaParser; PlantUML API &
126 casos sintéticos (alt/loop; formas/locais de chamada; criação; herança/polimorfismo; lambdas); avaliação funcional (“desenha corretamente”) \\

\textcite{baidada2016hybrid} &
Genérico; aplicações OO (GUI e multithread) &
UML Sequência (HLSD) &
\textbf{Híbrido} (Estático + Dinâmico); Compreensão/Redocumentação (comportamento) &
Estático: \textbf{CFG} para derivar valores de entrada por caso de uso $\rightarrow$ guiar execução; 
Dinâmico: traços múltiplos + \textbf{filtragem por pacote} (e.g., Swing) $\rightarrow$ 
Modelo intermediário: \textbf{Colored Petri Nets (CPN)} para fundir traços e detectar \texttt{seq/alt/opt/loop/par} $\rightarrow$ transformação CPN$\Rightarrow$UML &
Metodologia (sem ferramenta nomeada); IM em \textbf{CPN} &
Proposta conceitual; sem estudo industrial concluído; autores planejam avaliar em sistemas complexos \\

\textcite{leatongkam2017generating} &
Fortran OO; científico (computational science) &
UML Sequência (com suporte a coarrays) &
Estático; Compreensão/Redocumentação (comportamento) &
Regras formais (OMG UML) para \emph{lifelines}, \emph{messages}, \emph{conditional/decision/loop}, \emph{coarrays}; Extração sintática $\rightarrow$ árvore de classes $\rightarrow$ mapeamento $\rightarrow$ XMI &
Extensão do \textit{ForUML}; exporta \textbf{XMI}; visualização em \textit{ArgoUML} (avaliação de outras ferramentas) &
Work-in-progress; plano de testes com pacotes Fortran OO e avaliação por usuários; sem estudo industrial concluído \\

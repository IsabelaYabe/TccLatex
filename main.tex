...
Plano estático (objetivo: casos de uso a partir de Python)
...
include: subrotinas/serviços invocados por ≥k casos de uso candidatos (reuso sistemático no grafo de chamadas).
...
Por que estático é suficiente (e como ancoramos)
...
O mesmo trabalho mapeia herança estática para generalização: “generalization relations are obtained from inheritance and from public methods with the same signature in different classes.”【turn19file0L107-L110】
...
A transformação ATL inclui: “for each dependency between two public methods, create a dependency between the corresponding use cases.”【turn19file2L1-L7】
Fonte usada: regra Dependency2Dependency — base para nossas arestas de dependência/include.

Heurísticas estáticas (sem execução).
Os autores salientam que a técnica é estática (propagação em grafos de dados/controle) e que o “dinâmico” só melhora classificação de dependências; manteremos apenas o estático: “we perform a propagation analysis on the data-flow graph”【turn19file1L30-L37】 e, mais adiante, “dynamic analysis is used to classify dependencies…” (que aqui omitimos)【turn19file1L49-L57】.
Fonte usada: seção de análise — adotamos a parte de data-flow/AST e descartamos a parte dinâmica.

Embeddings estáticos guiados por AST.
(Liang & Huang, 2024) descrevem um modelo com dois encoders (um para semântica lexical e outro para estrutura de AST) para sumarização de código.
Assim aproveitamos os embeddings para melhorar a qualidade dos clusters.

Implicações para este trabalho. Propomos redocumentação semântica a partir de Python, combinando (i) AST como modelo intermediário (IM/IR) e (ii) embeddings sensíveis à estrutura (NFASRPR-TRANS) para inferir intenção e papel de métodos (docstrings, comentários e nomes). Casos de uso são inferidos por métodos de borda; generalizações vêm de herança/assinaturas homônimas; include decorre de subrotinas reutilizadas; extend de ramos opcionais. A saída é um Use Case Diagram em PlantUML. (Base: Pereira — extração estática por métodos públicos【turn19file0L101-L105】【turn19file0L107-L110】; Liang & Huang — embeddings orientados a AST【turn22file3L17-L33】.)

(B) Sua “arquitetura pipeline” (ajustada para 0\% dinâmico)

código → AST → embeddings+clusterização → regras estáticas (include/extend/atores) → PlantUML

De onde vem: mapeamento “método público → caso de uso” e “chamada entre públicos → dependência” (Pereira)【turn19file0L101-L105】【turn19file2L1-L7】; robustez semântica dos vetores via AST-encoder (Liang & Huang)【turn22file8L1-L5】.
...
include: função/método chamado por ≥k casos distintos; baixa complexidade ciclomática; sem guardas específicas.
...
Escoragem: score = α·(freq de reuso) + β·(centralidade no call graph) − γ·(força da guarda condicional) para decidir include vs. extend.

Saída: .puml gerado a partir do grafo.

Citações (trechos originais)

Recovering Use Case Diagrams from Object-Oriented Code: “Each public method of a class corresponds to a use case whose name is the same as the method name.”【turn19file0L101-L105】

Recovering Use Case Diagrams from Object-Oriented Code: “generalization relations are obtained from inheritance and from public methods with the same signature in different classes.”【turn19file0L107-L110】

Recovering Use Case Diagrams from Object-Oriented Code: “for each dependency between two public methods, create a dependency between the corresponding use cases.”【turn19file2L1-L7】

Recovering Use Case Diagrams from Object-Oriented Code: “we perform a propagation analysis on the data-flow graph”【turn19file1L30-L37】

Integrating Non-Fourier and AST-Structural Relative Position Representations…: “made up of two different types of encoders and one decoder…”【turn22file3L17-L21】; “…learn the structural information…”【turn22file3L29-L33】; “…allow the model to understand the semantics and syntax of the source code.”【turn22file8L1-L5】

Se topar, já escrevo o esqueleto do extrator (AST→grafo→PlantUML) e a rotina de scoring de include/extend.



Ajuste do plano estático (ancorado no artigo)
...
“Implicações para este trabalho” (versão enxuta, acadêmica, só estático)

Implicações para este trabalho. Propomos uma redocumentação estática de casos de uso para sistemas Python. O modelo intermediário (IM/IR) será a AST, da qual extrairemos: (i) casos de uso a partir de métodos/funções públicas; (ii) generalizações de UCs a partir de herança e homonímia de assinatura; e (iii) dependências entre UCs a partir de chamadas. Em coerência com Pereira et al., a classificação de dependências em <<include>>/<<extend>> é deliberadamente não automatizada (dinâmica fora de escopo); portanto, representamos “Dependency” e destacamos candidatos com heurísticas estáticas (reuso/guardas). Atores permanecem manuais. Para elevar o nível de abstração, aplicamos clusterização de UCs conforme as heurísticas de agrupamento do passo 4, gerando pacotes e UCs abstratos. A saída é um Use Case Diagram em PlantUML.

De onde veio cada regra (artigo + citação original em inglês):

Use case from public method: “Each public method of a class corresponds to a use case…” (Pereira et al.)

Generalization from inheritance + same-name: “For each pair of same-name methods… a generalization between use cases… is generated.”

Dependency from calls (sem tipar estáticamente): “Each message sent to an object corresponds to a dependence between use cases. The kind of relationship, include or extend, may be inferred by a dynamic analysis.”

Atores não automáticos: “actors can not be automatically inferred.”

Clusterização para alto nível: “use cases may be clustered in single more abstract use cases by applying simple heuristics.”

Concretizando no seu pipeline

Código → AST (IM/IR): ast/LibCST; extraia: classes, heranças, visibilidade/padrões “público”, assinaturas, chamadas. (Base nos “Steps 1–2” do artigo.)

UCs: um por função/método público (nome = Classe.metodo).

Arestas “Dependency”: de m → g para cada chamada p.g() encontrada em m.

Generalização de UCs: herança + homônimo.

Atores: curadoria manual (ou mapeamento externo); não automatizar.

Clusterização: agrupe UCs seguindo as heurísticas do passo 4 (ex.: um UC que inclui exclusivamente os demais do grupo → UC abstrato).

Geração: exporte PlantUML (UCs, Dependencies, Generalizations; include/extend apenas como tags candidatos, não relações formais).

\section{Código-fonte → AST}

Adotamos \textcite{LiangHuang2024ASTLLM} como referência para integrar duas representações posicionais — não-Fourier (semântica) e estrutural (AST) — no pipeline \emph{código→AST→modelo}, usando a AST como modelo intermediário (IM/IR). O modelo NFASRPR-TRANS emprega simultaneamente o \emph{Gaussian Embedder} e o \emph{Tree Positional Encoding} para que o encoder aprenda, respectivamente, relações semânticas entre tokens e relações estruturais induzidas pela árvore sintática.

\paragraph{Workflow.} O processo compreende: (i) pré-processamento de código/sumários; (ii) geração da AST; (iii) cálculo das duas representações relativas; (iv) alimentação dos encoders; (v) geração do resumo no \emph{decoder}.

\begin{figure}
  \centering
  \includegraphics[width=0.8\textwidth]{Images/draw/NFASRPR-TRANS.png}
  \caption{Fluxo de trabalho de sumarização do código-fonte do NFASRPR-TRANS.}
  \label{fig:fluxo_nfasrpr_trans}
\end{figure}

\paragraph{Pré-processamento e AST.} Para mitigar palavras fora do vocabulário, tokens em CamelCase/snake\_case são segmentados antes do \emph{word embedding}. As ASTs são extraídas via \texttt{javalang} (Java) e módulo \emph{attokens} (Python).

\paragraph{Representação não-Fourier.} A posição relativa entre tokens é codificada pelo \emph{Gaussian Embedder}, com \(\sigma\) escolhido log-linearmente em \([10^{-4},10^{-1}]\); \(\sigma\) controla o \emph{trade-off} memorização/generalização. Adicionalmente, a injeção da posição é feita \emph{em cada camada} do encoder de sequência.

\paragraph{Representação estrutural (AST).} Empregamos a \emph{Tree Positional Encoding} com travessia \emph{Pre-order Reverse}. O procedimento é: (i) gerar a sequência \emph{pre-order} da AST para fixar os índices dos nós; (ii) reverter essa sequência; e (iii) codificá-la pelos vetores posicionais, preservando o caminho  da raiz para o nó. Ao inverter a ordem, o modelo passa a “ler” primeiro os parâmetros e depois os operadores, interpretando a sequência como “os \emph{subjects} executam a ação do \emph{verb}”, o que aproxima a organização da AST da estrutura frasal “Sujeito + Verbo” em linguagem natural, sem perder a informação de programação (índices \emph{pre-order}) que imita o processamento feito por compiladores. Note que apenas a ordem de leitura é invertida; os índices atribuídos aos nós pela travessia em pré-ordem e seus vetores de caminho  da raiz ao nó são preservados (índices \emph{pre-order}). Na prática, essa opção melhora a legibilidade e a adequação semântica das sentenças geradas em funções com estrutura complexa.

\paragraph{Arquitetura e decodificação.} O sistema possui dois encoders (SC-Seq com Gaussian; AST-RP com Si-SAN e Tree) e um \emph{decoder} Transformer padrão com \emph{greedy} durante o teste, que seleciona a palavra com maior probabilidade.

\paragraph{Evidência empírica.} Em Java e Python, NFASRPR-TRANS supera os \emph{baselines} nas cinco métricas (2–10\%) e converge mais rapidamente.

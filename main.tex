...
\begin{figure}
  ...
  \label{fig:diagrama de sequencia da construção do TNode}
\end{figure}
...
\begin{figure}
  \centering
  \includegraphics[width=0.8\textwidth]{drawio/Images/AST_Model_Plugins-ast_api_package.drawio.png}
  \caption{Arquitetura modular do ASTCore.}
  \label{fig:arquitetura_astcore}
\end{figure}

\subsubsection{Pipeline de análise}

Do ponto de vista operacional, o fluxo de implementação do ASTCore segue as seguintes etapas:

\begin{enumerate}
  \item \textbf{Descoberta de arquivos}: a função \texttt{analyze\_path} recebe um caminho raiz de projeto e itera sobre os arquivos \texttt{.py}. Para cada arquivo, o código-fonte é lido por uma função utilitária (\emph{wrapper} de \texttt{Path.read\_text}) que tenta múltiplas codificações e retorna o texto bruto.
  \item \textbf{Construção da AST nativa}: o texto é convertido em uma AST do Python via \texttt{ast.parse}. Essa AST é a ``fonte de verdade'' sintática sobre a qual o ASTCore opera.
  \item \textbf{Inicialização do contexto}: é criado um objeto \texttt{Ctx} com as linhas do arquivo (\texttt{lines}), o mapeamento de comentários por linha (\texttt{comments\_by\_line}) e os caminhos \texttt{root\_path} e \texttt{file\_path}. Esse contexto é compartilhado por todos os passes.
  \item \textbf{Travessia da AST (\texttt{walk\_module})}: o módulo \texttt{astcore.walker} percorre a AST segundo a estratégia selecionada (por exemplo, \texttt{recursive\_pre}). A cada nó visitado, é criado um \texttt{TNode} contendo a referência ao nó original (\texttt{py\_node}) e informações estruturais básicas (\texttt{lineno}, \texttt{end\_lineno}, etc.).
  \item \textbf{Aplicação dos passes}: para cada \texttt{TNode}, o \texttt{PassRegistry} aplica em sequência os plugins carregados (\texttt{load\_pass\_plugins}). Cada plugin lê e atualiza campos específicos do \texttt{TNode}, respeitando o princípio de separação de preocupações.
  \item \textbf{Empacotamento dos resultados}: ao final da análise de um arquivo, o módulo \texttt{service} cria instâncias de \texttt{FileAnalysis} (arquivo, contexto, lista de \texttt{TNode}s e versão serializada de cada nó). Para uma análise completa, os resultados são agrupados em um \texttt{AnalysisResult} contendo a estratégia utilizada e a lista de arquivos analisados.
  \item \textbf{Serialização}: a função \texttt{export\_json} converte cada \texttt{TNode} em um dicionário JSON-\emph{friendly}, preservando tanto os metadados estruturais quanto os campos enriquecidos pelos passes. Esse JSON é o insumo direto para as etapas seguintes do TCC (tokenização semântica, geração de embeddings e clusterização de candidatos a casos de uso).
\end{enumerate}
...
Para enriquecer essas conexões, cada aresta \((i, j)\) recebe um peso escalar \(w_{ij}\) calculado a partir de uma função gaussiana sobre a distância estrutural entre os nós. Inspirado em esquemas de \emph{relative position encoding} com kernels gaussianos para código-fonte, o peso é definido como
\[
w_{ij}
= \exp\!\left(
  ...
  \right),
\]
em que \(p_i\) e \(p_j\) podem representar a profundidade na AST ou o número de linha no arquivo, e \(\sigma\) é um hiperparâmetro de escala. Essa lógica é implementada nas funções \texttt{gaussian\_edge\_weights} e \texttt{gaussian\_edge\_weights\_combined}. A versão combinada utiliza duas dimensões (profundidade e linha), agregando os respectivos pesos (por exemplo, via produto ou média), de modo que a proximidade sintática e a proximidade no código-fonte contribuam conjuntamente para a força da aresta.

Dessa forma, o grafo deixa de ser puramente não ponderado e passa a carregar um campo contínuo de relacionamentos estruturais: nós próximos (mesma região da árvore ou do arquivo) recebem pesos mais altos, enquanto nós distantes são atenuados.

\subsubsection{Entrada para a GNN}

Ao final dessa etapa, o modelo dispõe de três elementos:

\begin{itemize}
  \item \(\,X \in \mathbb{R}^{N \times d}\): embeddings iniciais de cada \texttt{TNode};
  \item \(\,\texttt{edge\_index} \in \mathbb{N}^{2 \times E}\): topologia do grafo da AST;
  \item \(\,\texttt{edge\_weight} \in \mathbb{R}^{E}\): pesos gaussianos associados a cada aresta.
\end{itemize}

Esses tensores são passados para o modelo \texttt{CodeGNN}:
...
onde \texttt{node\_embs} representa as embeddings contextuais refinadas de cada nó após a propagação de mensagens, e \texttt{graph\_emb} é obtido por um \emph{pooling} global, produzindo uma representação vetorial compacta do arquivo como um todo.
